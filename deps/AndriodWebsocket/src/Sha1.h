...
#ifndef V8DEBUGGER_SHA1_H
#define V8DEBUGGER_SHA1_H
 ...
 */
    ...
    inline std::array<uint8_t, 20> final()
    {
        ...
        for(int i = 0; i < 5; i++){
            ret[4*i]   = digest[i] >> 24;
            ret[4*i+1] = digest[i] >> 16;
            ret[4*i+2] = digest[i] >> 8;
            ret[4*i+3] = digest[i] >> 0;
        }
        return ret;
    }
